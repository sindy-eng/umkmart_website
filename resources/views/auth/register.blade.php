...
<head>
    ...
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn{width:100%;padding:13px;background:#f97316;color:#fff;border:none;border-radius:12px;font-size:.95rem;font-weight:700;font-family:inherit;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:1.5rem}
...
<div class="wrap">
    <div class="left">
        <div class="left-inner">
            ...
            <p class="tagline">Bergabunglah dengan UMKMART dan kelola bisnis UMKM Anda lebih efisien</p>
            <div class="features">
                <div class="feat">
    ...
    <div class="right">
        <div class="form-wrap">
            ...
            <div class="fh">
                ...
                <p>Daftar untuk mulai mengelola bisnis Anda</p>
            </div>
            <form method="POST" action="{{ route('register') }}">
                @csrf
                <div class="fg">
                    <label class="fl" for="name">Nama Lengkap</label>
                    <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" class="fi @error('name') err @enderror" placeholder="Nama lengkap Anda">
                    @error('name')<p class="fe">{{ $message }}</p>@enderror
                </div>
                <div class="fg">
                    <label class="fl" for="email">Email</label>
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" class="fi @error('email') err @enderror" placeholder="nama@email.com">
                    @error('email')<p class="fe">{{ $message }}</p>@enderror
                </div>
                <div class="fg">
                    <label class="fl" for="password">Password</label>
                    <input id="password" type="password" name="password" required autocomplete="new-password" class="fi @error('password') err @enderror" placeholder="Minimal 8 karakter">
                    @error('password')<p class="fe">{{ $message }}</p>@enderror
                </div>
                <div class="fg">
                    <label class="fl" for="password_confirmation">Konfirmasi Password</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password" class="fi" placeholder="Ulangi password">
                </div>
                <button type="submit" class="btn">
                    <svg style="width:20px;height:20px" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                    Daftar Sekarang
                </button>
            </form>
